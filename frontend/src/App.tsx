...
import { Play, Download, Mic, Settings, Music, RefreshCw, Volume2, Copy, X, Sparkles, FileJson } from 'lucide-react';
...
export default function App() {
  ...
  const handleSelectItem = (audio: AudioItem) => {
    ...
    // Nếu có metadata JSON, tự động điền dữ liệu
    if (audio.metadata) {
        ...
        <section className="bg-white p-8 rounded-[32px] border border-orange-100 shadow-lg">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="space-y-6">
              <div>
                <label className="block text-[10px] font-black text-orange-300 uppercase tracking-widest mb-4">Giới tính & Độ tuổi</label>
                <div className="grid grid-cols-2 gap-2">
                  <select value={gender} onChange={(e) => setGender(e.target.value)} className="bg-orange-50 border border-orange-100 rounded-xl p-3 text-xs font-bold text-orange-800 outline-none w-full appearance-none hover:border-orange-400 transition-colors cursor-pointer">
                    {GENDERS.map(g => <option key={g.value} value={g.value}>{g.label.split(' ')[0]}</option>)}
                  </select>
                  <select value={age} onChange={(e) => setAge(e.target.value)} className="bg-orange-50 border border-orange-100 rounded-xl p-3 text-xs font-bold text-orange-800 outline-none w-full appearance-none hover:border-orange-400 transition-colors cursor-pointer">
                    {AGES.map(a => <option key={a.value} value={a.value}>{a.label.split(' ')[0]}</option>)}
                  </select>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-6">
              <div>
                <label className="block text-[10px] font-black text-orange-300 uppercase tracking-widest mb-4">Giọng (Accent)</label>
                <select value={accent} onChange={(e) => setAccent(e.target.value)} className="w-full bg-orange-50 border border-orange-100 rounded-xl p-3 text-xs font-bold text-orange-800 outline-none appearance-none hover:border-orange-400 transition-colors cursor-pointer">
            ...
            <div className="space-y-6">
              <div>
                <label className="block text-[10px] font-black text-orange-300 uppercase tracking-widest mb-4">Cao độ & Phong cách</label>
                <div className="grid grid-cols-2 gap-2">
                  <select value={pitch} onChange={(e) => setPitch(e.target.value)} className="bg-orange-50 border border-orange-100 rounded-xl p-3 text-xs font-bold text-orange-800 outline-none w-full appearance-none hover:border-orange-400 transition-colors cursor-pointer">
                    {PITCHES.map(p => <option key={p.value} value={p.value}>{p.label.split(' ')[0]}</option>)}
                  </select>
                  <select value={style} onChange={(e) => setStyle(e.target.value)} className="bg-orange-50 border border-orange-100 rounded-xl p-3 text-xs font-bold text-orange-800 outline-none w-full appearance-none hover:border-orange-400 transition-colors cursor-pointer">
                    {STYLES.map(s => <option key={s.value} value={s.value}>{s.label.split(' ')[0]}</option>)}
                  </select>
                </div>
              </div>
            </div>
